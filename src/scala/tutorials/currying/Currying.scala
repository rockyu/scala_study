...
object Currying extends App {
  ...
  // out: 2, 4, 6, 8
  println(filter(nums, modN(2)))   // modN(2) yields a function of type Int => Boolean

  // out: 3, 6
  println(filter(nums, modN(3)))

  // Note: method modN is partially applied in the two filter calls; i.e. only its first
  // argument is actually applied. The term modN(2) yields a function of type Int => Boolean
